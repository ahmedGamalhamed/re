...
class FootlockerMonitor(threading.Thread):
	...
	def __init__(self, proxies, config, item):
		...
		self.sale_webhooks_url        = config['mornitor_urls'][0]['webhooks_url']
		...
		while True:
			try:
				if self.search_url == "keyword_search":
					...
					response = self.scraper.get(self.keyword_search_url, params=params, timeout=5)
				else:
					response = self.scraper.get(self.search_url, timeout=5)
			except Exception as e:
				log('f', str(repr(e)))
				continue
			if (response.status_code == 200):
				tree_html = BS(response.content, features="lxml")
				items = tree_html.find_all('div', 'fl-category--productlist--item')
				for item in items:
					product_url = item.find('a')['href']
					temp = {}
					temp.update({'url': product_url})
					try:
						self.get_ajax_url(temp)
					except Exception as e:
						print_error_log("Get Product ajx:" + str(e))
				break
			else:
				print_error_log("Get Product ajx:" + str(response.status_code))
				sleep(2)
				continue
	...
	def get_ajax_url(self, url):
		...
		try:
			res = self.scraper.get(url['url'], timeout=10)
		except Exception as e:
			print_error_log("Fail get product ajax URL: [{}]".format(url))
			return
		...
		if "All rights reserved" in res.text:
			...
			temp.update({'product_url': url['url']})
	...
	def ajax_response_parser(self, url):
		...
		try:
			...
			log('f', "Get Product Detail : {}".format(repr(e)))
			return
...
if (__name__ == "__main__"):
	...
	for item in config['mornitor_urls']:
		newthread = FootlockerMonitor(proxies, config, item)
		newthread.start()
		sleep(10)
