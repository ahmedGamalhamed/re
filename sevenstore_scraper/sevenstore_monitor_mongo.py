...
def send_discord(product, webhooks_url):
	...
	embed.set_author(name='SevenStore' , icon_url="https://the7.io/shop/wp-content/uploads/sites/7/2017/04/seven-store-logo-hd.png")
	...
	embed.set_footer(text='SevenStore Monitor ', icon_url="")
...
class SevenStoreScraper(threading.Thread):
	...
	def __init__(self, proxies, config, item):
		...
		self.all_in_ones_webhooks_url   = config['all_in_ones_webhooksurl']
	...
	def search_sevenstore(self, type, param):
		headers = {
		...
		try:
			...
			for item in product_main_container.find('div').find_all('div', 'nodecor'):
				url = item.find('a').get('href')
				pid = item.find('span').get('data-pid')
		...
		if response.status_code == 200:
			...
			try:
				...
				for key in json_data.keys():
					...
					if pid == data.get('pf_id') and data.get('sell') == "true":
						size = "UK" + data.get('option2')
						if data.get('sale_item') == 'true':
							is_sale = True
						sizes.append(size)
						price   = data.get('price').replace('Â', '')
						wasprice = data.get('wasprice').replace('Â', '')
						name    = data.get('name')
				if not name:
					return
				...
				log('f', "Incorrect Product URL:[{}]".format(url))
				return False
			except Exception as e:
				print_error_log("URL:" + url + " " + str(e))
		else:
			#log('f', "Get Product Detail --> Status code:" + str(response.status_code))
	...
	def add_to_product_mongodb(self, product, type):
		...
		try:
			...
			log('s', "Found New Product <{}> in SevenStore UK [{}].".format(product['prod_name'], type.upper()))
			alert = "New"
			...
			if (len(old_prod_sizes) == 0 and len(product['prod_size']) > 0) or len(old_prod_sizes) + 1 < len(product['prod_size']):
				log('s', "Found Restocked Product <{}>.in SevenStore UK [{}]".format(product['prod_name'], type.upper()))
				alert = "Restock"
				old_status = alert
	...
	def run(self):
		while True:
			try:
				log('i', "Start SevenStore Scraping from [{}]...".format(self.item.get('type')))
	...
	def start_scrape(self, item):
		if item.get('type') == "keyword":
			for keyword in item.get('search_keywords'):
				self.search_sevenstore("keyword", keyword)
		else:
			for url in self.item.get('search_urls'):
				self.search_sevenstore("url", url)
...
if (__name__ == "__main__"):
	...
	for item in config['mornitor_urls']:
		...
		newthread = SevenStoreScraper(proxies, config, item)
		newthread.start()
		sleep(2)
