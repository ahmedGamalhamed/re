...
class SevenStoreScraper(threading.Thread):
	...
	def add_to_product_mongodb(self, product, type):
		...
		try:
			...
			log('s', "Found New Product <{}> in SevenStore [{}].".format(product['prod_name'], type.upper()))
			alert = "New"
	...
	def run(self):
		while True:
			try:
				self.visit_homepage()
				log('i', "Start SevenStore Scraping from [{}]...".format(self.keyword))
...
if (__name__ == "__main__"):
	...
	newthread = SevenStoreScraper(proxies, config, all_in_ones)
	newthread.start()
	sleep(2)
