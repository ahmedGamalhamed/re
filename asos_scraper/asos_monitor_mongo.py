...
class AsosScraper(threading.Thread):
    ...
    def search_footasylum(self, type, param):
        while True:
            ...
            try:
                if type == "keyword":
                    params  = { "page": "1", "q" : param }
                    url = self.item['keyword_search_url']
                    response = self.scraper.get(url, params=params, timeout=5, proxies=self.proxy)
                else:
                    url = param
                ...
                continue
        try:
            for item in BS(response.content, features='lxml').find_all("article"):
                url = item.find('a').get('href')
                self.get_product_detail(url)
                # break#TODO
        except Exception as e:
            print_error_log("Get Product List: " + str(e))
    ...
    def run(self):
        while True:
            try:
                log('i', "Start Asos Scraping from [{}]...".format(self.item.get('type')))
...
if (__name__ == "__main__"):
    ...
    for item in config['mornitor_urls']:
        ...
        newthread = AsosScraper(proxies, config, item)
        newthread.start()
        sleep(2)
