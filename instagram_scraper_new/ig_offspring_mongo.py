...
class InstagramScraper():
	def __init__(self, **kwargs):
		...
		# posttime_idx = IndexModel([("post_time", DESCENDING)], name='posttime_idx')
		# self.collection.create_indexes([ posttime_idx ])
	...
	def start_post_hash_scrape(self, username, dbname):
		...
		while True:
			...
			try:
				response = self.scraper.get(start_url, headers=self.headers, proxies=proxies, timeout=5)
			except Exception as e:
			...
			for item in lists:
				...
				if alert and (alert == "New") :
					try:
						...
						log('f', "Get post detail {}".format(repr(e)))
						continue
		except Exception as e:
			log('f', "Fail get {} --> Username:[{}] : URL: {} {}".format(dbname.upper(), username, response.url, repr(e)))
	...
	def get_post_detail(self, postlink, username, webhooks_url):
		...
		while True:
			proxies = get_proxy(self.proxies)
			try:
				response = self.scraper.get(post_url, timeout=5, proxies=proxies, headers=self.headers)
			except:
				try:
					response = self.scraper.get(post_url, timeout=5, proxies=proxies, headers=self.headers)
				except Exception as e:
