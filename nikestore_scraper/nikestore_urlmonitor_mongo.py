...
class NikeStoreScraper(threading.Thread):
    ...
    def __init__(self, proxies, config, all_in_ones):
        ...
        # prodname_idx = IndexModel([("prod_name", TEXT)], name='prodname_idx')
        # prod_updatedtime_idx = IndexModel([("prod_updatedtime", DESCENDING)], name='prod_updatedtime_idx')
        # prod_shortcode_idx = IndexModel([("prod_shortcode", ASCENDING)], name='prod_shortcode_idx')
        # self.collection.create_indexes([ prodname_idx, prod_updatedtime_idx, prod_shortcode_idx ])
    ...
    def get_product_detail(self, url):
        ...
        if response.status_code == 200:
            product = {}
            try:
                # with open('dummy/detail.html', 'wb') as f:
                #     f.write(response.content)
                ...
                # with open('dummy/prod_detail_key_error.json', 'w', encoding="utf-8") as f:
    ...
    def run(self):
        while True:
            try:
                log('i', "Start NikeStore Scraping from [{}]...".format(self.keyword))
                self.visit_homepage()
...
if (__name__ == "__main__"):
    ...
    newthread = NikeStoreScraper(proxies, config, all_in_ones)
    newthread.start()
    sleep(2)
